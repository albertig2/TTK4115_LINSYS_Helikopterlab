...
typedef RT_MODEL_simulink_kalmanfilte_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * simulink_kalmanfilter(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(simulink_kalmanfilter_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    stream_shutdown(simulink_kalmanfilter_DW.StreamCall1_Stream);
  }

  hil_task_stop(simulink_kalmanfilter_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: simulink_kalmanfilter -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        result = hil_task_start
          (simulink_kalmanfilter_DW.HILReadEncoderTimebase_Task, (t_clock)
           simulink_kalmanfilter_P.HILReadEncoderTimebase_Clock, 500.0, -1)
          ;
        if (result == 0) {
