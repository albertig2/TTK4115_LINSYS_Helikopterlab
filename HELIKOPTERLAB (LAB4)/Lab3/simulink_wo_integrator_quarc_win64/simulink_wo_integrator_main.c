...
typedef RT_MODEL_simulink_wo_integrat_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * simulink_wo_integrator(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(simulink_wo_integrator_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    stream_shutdown(simulink_wo_integrator_DW.StreamCall1_Stream);
  }

  hil_task_stop(simulink_wo_integrator_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://simulink_wo_integrator:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: simulink_wo_integrator -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        result = hil_task_start
          (simulink_wo_integrator_DW.HILReadEncoderTimebase_Task, (t_clock)
           simulink_wo_integrator_P.HILReadEncoderTimebase_Clock, 500.0, -1)
          ;
        if (result == 0) {
